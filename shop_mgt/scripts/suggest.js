...
var getFunctionsUrl = "ajax_query/suggest.php?keyword=";
...
function init()
{
	...
	var oKeyword = document.getElementById("customer_name");
...
function addNewWordToCache(new_word)
{
        if( !oCache )
                oCache = new Object();
        ...
        {
                var letters = new_word.substring(0, i+1).toLowerCase();
                if( !oCache[letters] )
                {
			break;
                }
                ...
                {
                        list_len = oCache[letters].length;
			for(var j=0; j<list_len; j++)
			{
				if(oCache[letters][j] == new_word)
					return;
			}
                        oCache[letters][list_len] = new_word;
			oCache[letters].sort(sort_caseinsensitive);
                }
        }
}
...
function addToCache(keyword, values)
{
	...
	for(i=0; i<values.length; i++)
	{
		...
		//alert("1. oCache[" + keyword + "][i] = " + values[i]);
	}
}
/*
...
function getSuggestions(keyword, isBackspacePressed)
{
	keyword = keyword.toLowerCase();
	...
	if(keyword != "" && !isKeyUpDownPressed)
	{
		...
		if(isInCache == true)
		{
			...
			if(oCache[keyword].length == 0 )
				hideSuggestions();
			else
				displayResults(keyword, oCache[keyword], isBackspacePressed);
		}
			...
			if(xmlHttp)
			{
				try
				{
							...
							xmlHttp.readyState == 0)
					{
						httpRequestKeyword = keyword;
						userKeyword = keyword;
						userBackspaceFlag = isBackspacePressed;
						xmlHttp.open("GET",
					...
					else
					{
						...
						timeoutId = setTimeout("getSuggestions(userKeyword, userBackspaceFlag);", 500);
					}
				}
				catch(e)
				{
...
function updateSuggestions()
{
	...
	if(httpRequestKeyword == userKeyword)
	{
		...
		else
		{
			...
			displayResults(httpRequestKeyword, nameArray, false);
		}
	}
	else
	{
...
function displayResults(keyword, results_array, isBackspacePressed)
{
	// start building the HTML table containing the results
	var div = "<table id='suggestion_box' class='no_border' cellspacing = '0' cellpadding='0'>";
	// if the searched for keyword is not in the cache then add it to the cache
	if((!oCache && keyword) || (!oCache[keyword] && keyword))
	...
	else
	{
		...
		for (var i=0; i<oCache[keyword].length; i++)
		{
			...
			cust_name = oCache[keyword][i];
			//alert("2. cust_name = " + cust_name);
			// set the string link for the for the current function
			// to the name of the function
			cust_nameLink = cust_name;
			// replace the _ with - in the string link
			while(cust_nameLink.indexOf("_") !=-1)
				cust_nameLink = cust_nameLink.replace("_","-");
			...
			div += "<tr id='tr" + i + "' onmouseover='handleOnMouseOver(this);' " +
				"onmouseout='handleOnMouseOut(this);' onclick='hideSuggestions();updateKeywordValue(this)'>" +
				"<td align='left'>" ;
			...
			if(cust_name.length <= suggestionMaxLength)
			{
				// bold the matching prefix of the function name and of the keyword
				div += "<b>" +
					cust_name.substring(0, httpRequestKeyword.length) +
					"</b>"
					div += cust_name.substring(httpRequestKeyword.length,
							cust_name.length) +
					"</a></td></tr>";
			}
			else
			{
				...
				else
				{
					...
					div += "<b>" +
						cust_name.substring(0,suggestionMaxLength) +
						"</b></td></tr>"
				}
			}
		}
	...
	if(results_array.length > 0 && !isBackspacePressed)
		autocompleteKeyword();
}
...
function checkForChanges(e)
{
	...
	var keyword = document.getElementById("customer_name").value;
	...
	if (e.type == "keyup")
	{
		if(code == 8)
		{
			//If backspace is pressed, do not autocomplete
	...
	if((userKeyword != keyword) &&
		...
		getSuggestions(keyword, isBackspacePressed);
}
		...
		else
		{
			...
			if(code == 13)
			{
				...
				if(position>=0)
				{
					...
					unSelect(document.getElementById("customer_name"));
				}
			}
			else
			{
...
{
	// retrieve the keyword object
	var suggested_name = document.getElementById("customer_name");
	if (document.all) // IE Stuff
	{
		sug_name = oTr.firstChild.innerText;
	}
	else
	{
		sug_name = oTr.firstChild.textContent;
	}

        suggested_name.value = unescape(sug_name);
}

/*Removes any tags found in the string */
